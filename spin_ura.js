...
for (let i=0; i<4; i++)
{
    ...
    obj.style.left = width * (0.21 + 0.15 * i);
...
for (let i=0; i<3; i++)
{
    ...
    let src = "image/spin_6.png";
    obj.src = src;
    obj.style.opacity = 1;
    if (i == 0) obj.style.transform = "rotate(180deg)";
    ans1Obj.push(obj);
    camera.appendChild(obj);
}
...
function spin(event,i, j)
{
    ...
    if (i == 0)
    {
        formula1 = spinObjs[0][0].val + spinObjs[0][1].val + spinObjs[0][2].val + spinObjs[0][3].val;
        ans1 = Function('"use strict";return (' + formula1 + ")")() + "";
        for (let i=0; i<ans1.length; i++)
        {
            if (ans1[i] == 1 || ans1[i] == 2 || ans1[i] == 3 || ans1[i] == 6)
            {
                changeAnimation(ans1Obj[i], "image/spin_" + ans1[i] + ".png", false, 0, i);
            }
            else if (ans1[i] == 0 || ans1[i] == 4 || ans1[i] == 5 || ans1[i] == 7 || ans1[i] == 8)
            {
                changeAnimation(ans1Obj[i], "image/spin_" + ans1[i] + "_ura.png", false, 0, i);
            }
            else if (ans1[i] == 9)
            {
                changeAnimation(ans1Obj[i], "image/spin_" + 6 + ".png", false, 180, i);
            }
            else if (ans1[i] == "-")
            {
                changeAnimation(ans1Obj[i], "image/spin_" + 1 + ".png", false, 90, i);
            }
            
        }
        for (let i=ans1.length; i<3; i++)
        {
            changeAnimation(ans1Obj[i], "", false, 0, i);
        }
    }
...
function changeAnimation(obj, nextSrc, changed, deg, i)
{
    let c = changed;
    ...
    else
    {
        ...
        {
            obj.src = nextSrc;
            if (nextSrc == "") obj.style.display = "none";
            else obj.style.display = "inline";
            obj.style.transform = "rotate(" + deg + "deg)";
            obj.style.left = width * (0.51 + 0.15 * (i - ans1.length / 2));
            c = true;
        }
    }
    window.setTimeout(() => {changeAnimation(obj, nextSrc, c, deg, i);}, 10);
}
